...
the RHEL 7.2 upgrade is performed so that the backup can be used afterwards
...
def backupPreparation(upgradeWorkingDir, programParentDir):
	...
	try:
		osArchiveBackup = upgradeResourceDict['osArchiveBackup'].split(',')
		osRestorationBackup = upgradeResourceDict['rhelOSRestorationBackup'].split(',')
		sapRestorationBackup = upgradeResourceDict['sapRestorationBackup'].split(',')
	...
	'''
        The first time through we copy the additional RPMs needed for the post-upgrade to the pre-upgrade working directory.
	'''
        addOnSoftwareRPMDir = upgradeWorkingDir + '/addOnSoftwareRPMS'

        if not os.path.exists(addOnSoftwareRPMDir):
                try:
                        addOnSoftwareRPMSDir =  programParentDir + '/addOnSoftwareRPMS'
                        shutil.copytree(addOnSoftwareRPMSDir, addOnSoftwareRPMDir)
                except OSError as err:
                        logger.error('Unable to copy the additional RPMs from \'' + addOnSoftwareRPMSDir + '\' to \'' +  addOnSoftwareRPMDir + '\'.\n' + str(err))
                        print(RED + 'Unable to copy the additional RPMs from \'' + addOnSoftwareRPMSDir + '\' to \'' +  addOnSoftwareRPMDir + '\'; fix the problem and try again; exiting program execution.' + RESETCOLORS)
                        exit(1)
	...
	if getSAPUserLoginData(upgradeWorkingDir, sidList):
		osRestorationBackup.append('/home/sapadm')

	#Save out HANA fstab data.
	getHANAFstabData(upgradeWorkingDir, fstabHANAEntries)

	'''
	Get an updated list of the SAP HANA files/directories to backup.
	'''
	sapHANABackup = updateSAPHANABackupList(sapRestorationBackup)

	#Check to make sure there is enough disk space for the backup.
	backupItems = ' '.join(osRestorationBackup) + ' ' + ' '.join(osArchiveBackup) + ' ' + ' '.join(sapHANABackup)
	checkDiskspace(backupItems)

	#Save out network information.
	createNetworkInformationFile(upgradeWorkingDir)

	return [osRestorationBackup, osArchiveBackup, sapHANABackup]

#End backupPreparation(upgradeWorkingDir, programParentDir):


'''
This function updates the list of SAP HANA related files to backup.
'''
def updateSAPHANABackupList(sapRestorationBackup):
	sapHANABackup = []

        logger = logging.getLogger('coeOSUpgradeLogger')

        logger.info('Updating the list of SAP HANA related files/directories that will be backed up.')

	#Exclude a file/directory from the backup if it does not exist.
	for item in sapRestorationBackup:
		if os.path.exists(item):
			sapHANABackup.append(item)
	...
	sapHANABackup += out.splitlines()

	logger.info('The list of SAP HANA related files/directories to backup was determined to be: ' + str(sapHANABackup) + '.')

        logger.info('Done updating the list of SAP HANA related files/directories that will be backed up.')

	return sapHANABackup

#End updateSAPHANABackupList(sapRestorationBackup):
...
def getGlobalIniFiles(upgradeWorkingDir):
	...
	logger.info('Getting the global.ini files for the currently active SIDs as identified by \'/usr/sap/sapservices\'.')
	...
	logger.info('Done getting the global.ini files for the currently active SIDs as identified by \'/usr/sap/sapservices\'.')
...
'''
This function is used to create the NIC MAC address cross reference file that will be used for reference after the
upgrade to make sure the NIC mapping is correct.
'''
def createNetworkInformationFile(upgradeWorkingDir):
	...
	for data in nicDataList:
		if 'HWaddr' in data and not 'bond' in data:
			try:
	...
	procBondingDir = '/proc/net/bonding'
		...
		#Get the list of currently active bond interfaces.
		...
		#Update the nicDict with corrected mac addresses.
		for bondName in activeBondsList:
			...
			for data in procBondingData:
				...
				if 'Permanent HW addr' in data:
					...
					#The NIC dictionary is updated with bond mac addresses from '/proc/net/bonding/bond#, since ifconfig will not report the correct mac address for bonded interfaces.
					nicDict[macAddress] = slaveInterface

		logger.info('The updated NIC dictionary was determined to be: ' + str(nicDict) + '.')

		#Update nic configuration file if it exsists with MAC address, since an upgrade results in mapping errors.
...
#End createNetworkInformationFile(nicDataFileDir):
	

'''
This function update a NIC's configuration file with its MAC address information
so that after an upgrade the NIC is still mapped to its original name.
'''
def updateNICCfgFiles(nicDict):
	...
	for macAddress in nicDict:
		...
		if os.path.exists(nicCFGFile):
			...
			'''
			A return code of '0' means that the line was present and uncommented and no action is necessary.
			Otherwise, the file is updated with the NIC's MAC address information.
			'''
			if result.returncode != 0:
...
osRestorationBackupList followed by the osArchiveBackupList, followed by sapRestorationBackupList.
'''
def createBackup(backupList, upgradeWorkingDir):
	...
	osRestorationBackupList = [archiveDir + '/' + hostname + '_OS_Restoration_Backup_For_RHEL_Upgrade_' + dateTimestamp + '.tar', archiveDir + '/' + hostname + '_OS_Restoration_Backup_For_RHEL_Upgrade_' + dateTimestamp + '.tar.gz', archiveDir + '/' + hostname + '_OS_Restoration_Backup_For_RHEL_Upgrade_' + dateTimestamp, 'OS restoration']

	osArchiveBackupList = [archiveDir + '/' + hostname + '_OS_Archive_Backup_For_RHEL_Upgrade_' + dateTimestamp + '.tar', archiveDir + '/' + hostname + '_OS_Archive_Backup_For_RHEL_Upgrade_' + dateTimestamp + '.tar.gz', archiveDir + '/' + hostname + '_OS_Archive_Backup_For_RHEL_Upgrade_' + dateTimestamp, 'OS backup']

	sapRestorationBackupList = [archiveDir + '/' + hostname + '_SAP_Restoration_Backup_For_RHEL_Upgrade_' + dateTimestamp + '.tar', archiveDir + '/' + hostname + '_SAP_Restoration_Backup_For_RHEL_Upgrade_' + dateTimestamp + '.tar.gz', archiveDir + '/' + hostname + '_SAP_Restoration_Backup_For_RHEL_Upgrade_' + dateTimestamp, 'SAP restoration']

	count = 0
	#Create a backup image of the directories/files to be restored.
	for backupData in backupList:
		if count == 0:
			backupReferenceList = osRestorationBackupList
		elif count == 1:
			backupReferenceList = osArchiveBackupList
		else:
	...
	backupISO = upgradeWorkingDir + '/' + hostname + '_Archive_Backup_For_RHEL_Upgrade_' + dateTimestamp + '.iso'
	backupISOMd5sumFile = upgradeWorkingDir + '/' + hostname + '_Archive_Backup_For_RHEL_Upgrade_' + dateTimestamp + '.md5sum'
...
def main():
	...
	upgradeWorkingDir = '/tmp/CoE_SAP_HANA_RHEL_Upgrade_Working_Dir'
	...
	logFile = logDir + '/RHEL_Pre-Upgrade_' + dateTimestamp + '.log'
	...
	#The backupList consists of the os restoration backup, the sap restoration backup, and the os archive backup.
