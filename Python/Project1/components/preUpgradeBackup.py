...
the SLES 12.1 or RHEL 7.2 upgrade is performed so that the backup can be used afterwards
...
'''
def backupPreparation(osDist, upgradeWorkingDir, programParentDir):
	...
	try:
		...
		if osDist == 'SLES':
			...
			restorationBackup = upgradeResourceDict['rhelRestorationBackup'].split(',')
	...
	#The first time through we copy the hp-scripting-tools RPM needed for the post-upgrade to the pre-upgrade working directory.
	hpScriptingToolsRPMDir = upgradeWorkingDir + '/hpScriptingToolsRPM'

	if not os.path.isdir(hpScriptingToolsRPMDir):
		try:
			os.mkdir(hpScriptingToolsRPMDir)
			...
			shutil.copy2(hpScriptingToolsRPMFile, hpScriptingToolsRPMDir)
		except OSError as err:
			print(RED + 'Unable to copy the hp-scripting-tools RPM file \'' + hpScriptingToolsRPMFile + '\' to \'' +  hpScriptingToolsRPMDir + '\'; fix the problem and try again; exiting program execution.\n' + str(err) + RESETCOLORS)
			exit(1)

	#The first time through we also copy the add on file archive needed for the post-upgrade to the pre-upgrade working directory.
	addOnFilesDir = upgradeWorkingDir + '/addOnFiles'

	if not os.path.isdir(addOnFilesDir):
		try:
			os.mkdir(addOnFilesDir)

			if processor == 'ivybridge':
				addOnFile = programParentDir + '/addOnFiles/' + upgradeResourceDict['ivyBridgeAddOnFiles']
			else:
				addOnFile = programParentDir + '/addOnFiles/' + upgradeResourceDict['haswellAddOnFiles']

			shutil.copy2(addOnFile, addOnFilesDir)
		except OSError as err:
			print(RED + 'Unable to copy the add on files archive \'' + addOnFile + '\' to \'' +  addOnFilesDir + '\'; fix the problem and try again; exiting program execution.\n' + str(err) + RESETCOLORS)
			exit(1)
	...
	#On servers that are not Serviceguard we check if SAP HANA is running and copy the global.int files to subdirectories in the working directory.
	...
	'''
	Save out SAP user information. Also, if sapadm is present and thier home 
	...
	createNetworkInformationFiles(osDist, nicDataFilesDir)

	return [restorationBackup, archiveBackup]

#End backupPreparation(osDist, upgradeWorkingDir, programParentDir):
	...
	#The user login data is stored in a seperate directory.
        if not os.path.isdir(userLoginDataDir):
                try:
                        ...
                        print(RED + 'Unable to create the pre-upgrade user login data directory \'' + userLoginDataDir + '\'; fix the problem and try again; exiting program execution.\n' + str(err) + RESETCOLORS)
                        exit(1)

	passwordFile = userLoginDataDir + '/password'
	...
	try:
		...
		for sid in sidList:
			...
			if sidadm in passwordDict:
				password.write(passwordDict[sidadm][0] + '\n')
	...
	print(GREEN + 'Getting SAP HANA related partition data from \'/etc/fstab\'.' + RESETCOLORS)
        ...
        if not os.path.isdir(fstabDataDir):
                try:
                        ...
                        print(RED + 'Unable to create the pre-upgrade fstab data directory \'' + fstabDataDir + '\'; fix the problem and try again; exiting program execution.\n' + str(err) + RESETCOLORS)
                        exit(1)
	...
	hostname = os.uname()[1]
        ...
        if not os.path.isdir(hostnameDir):
                try:
                        ...
                        print(RED + 'Unable to create the pre-upgrade hostname data directory \'' + hostnameDir + '\'; fix the problem and try again; exiting program execution.\n' + str(err) + RESETCOLORS)
                        exit(1)
...
def checkDiskspace(backupItems):
	...
	if backupSize > 3:
		logger.error('The current size \'' + str(backupSize) + '\' of the backup of \'' + backupItems + '\'; is greater than 3GB.')
		print(RED + 'The current size \'' + str(backupSize) + '\' of the backup of \'' + backupItems + '\'; exceeds 3GB; fix the problem and try again; exiting program execution.' + RESETCOLORS)
		exit(1)
...
'''
def createNetworkInformationFiles(osDist, nicDataFilesDir):
        logger = logging.getLogger('coeOSUpgradeLogger')
	...
	if osDist == 'SLES':
		command = 'ls ' + slesNetworkDir + '/ifcfg-bond[0-9]'
	else:
		command = 'ls ' + rhelNetworkDir + '/ifcfg-bond[0-9]'
...
#End createNetworkInformationFiles(osDist, nicDataFilesDir):
...
'''
def createBackup(backupList, osDist, upgradeWorkingDir):
        logger = logging.getLogger('coeOSUpgradeLogger')
	...
	restorationBackupList = [archiveDir + '/' + hostname + '_Restoration_Backup_For_' + osDist + '_Upgrade_' + dateTimestamp + '.tar', archiveDir + '/' + hostname + '_Restoration_Backup_For_' + osDist + '_Upgrade_' + dateTimestamp + '.tar.gz', archiveDir + '/' + hostname + '_Restoration_Backup_For_' + osDist + '_Upgrade_' + dateTimestamp, 'restoration']

	archiveBackupList = [archiveDir + '/' + hostname + '_Archive_Backup_For_' + osDist + '_Upgrade_' + dateTimestamp + '.tar', archiveDir + '/' + hostname + '_Archive_Backup_For_' + osDist + '_Upgrade_' + dateTimestamp + '.tar.gz', archiveDir + '/' + hostname + '_Archive_Backup_For_' + osDist + '_Upgrade_' + dateTimestamp, 'archive']
...
#End createBackup(backupList, osDist, upgradeWorkingDir):
...
def main():
	...
	upgradeWorkingDir = '/tmp/coeSapHANA' + osDist + 'UpgradeWorkingDir'

	#The first time through we copy all the files needed for the post-upgrade to the pre-upgrade working directory.
	if not os.path.exists(upgradeWorkingDir):
		try:
			...
			print(RED + 'Unable to copy the post-upgrade script files from \'' + programParentDir + '\' to \'' +  upgradeWorkingDir + '\'; fix the problem and try again; exiting program execution.\n' + str(err) + RESETCOLORS)
			exit(1)
	...
	backupList = backupPreparation(osDist, upgradeWorkingDir, programParentDir)

	backupISO = createBackup(backupList, osDist, upgradeWorkingDir)
