#!/bin/bash
# disable_early_alua_transition.sh: Disable early ALUA transition
...
#   the script with "--uninstall" option to clean the rcopy_early_alua_transition mvar settings
#
# - Defect(s) it works around: 198734
#
# Script supports --install, --uninstall and --verify options
# (1) --install option
#      - Script adds rcopy_early_alua_transition mvar to sysvars.init file in following format.
#        following format: rcopy_early_alua_transition=0
#
# (2) --uninstall option
#      - Script removes rcopy_early_alua_transition mvar entries from sysvars.init file.
#        rcopy_early_alua_transition=1
#
# (3) --verify option. It gets data below:
#
#      - Contents of sysvars.init file
#      - Current rcopy_early_alua_transition mvar value. If rcopy_early_alua_transition=0 then workaround is applied
#      - Event log messages with disable_early_alua_transition.sh pattern in last 8 hours

Version=1.00

# For EGA specify GA; for EMUx specify MUx
TPD_VERSIONS="3.2.2.MU[34]"
...
usage()
{
  local PROG=$(basename $0)
    ...
    echo "--install   : Installs mvar in sysvars.init file to disable early ALUA transition"
    echo "--uninstall : Uninstalls/Removes mvar from sysvars.init file to disable early ALUA transition"
    echo "--verify    : Verify whether workaround is installed"

    echo -e "\nNote: This Script needs to be 'uninstalled' after upgrade is completed or if upgrade is aborted."
...
disable_early_alua_transition()
{
  local OPT="$1"
  local DESCRIPTION="Disable early ALUA transitions, persistent across node reboot and/or upgrade"
  local DEFECTS="183833"
  local TPDVARTYPE=mvar
  local TPDVARNAME=rcopy_early_alua_transition
  local DEFAULTVAL=1 # Default value (For ex:- run tcli -e 'mvar show -n rcopy_early_alua_transition')
  local NEWVAL=0
...
disable_early_alua_transition $OPT
rval=$?
