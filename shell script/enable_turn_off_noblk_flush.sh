#!/bin/bash
# enable_turn_off_noblk_flush.sh: Script sets turn_off_noblk_flush global variable to '1' in all the nodes in cluster
#
# Detailed problem description:
# A sub-16K write to a TDVV creates a page whose space is from local SD. Then LD flusher picks it up, creates buddy cmp and sends the buddy cmp for flushing.
# At this time, the page is on lru_ldwrt_hdr. Then a new full 16K write happens, dedup information is set up for the page and the page is kept on lru_ldwrt_hdr.
#
# The the TDVV is closed. During close, we drop the dedup information for all the pages of the TDVV and they are moved to WS or CLEAN LRU.
# For this case, the page is dirty so it is moved to WS LRU. Then the VV close process moves all the dirty pages to priority WS LRU.
# Later on, the page is flushed out successfully and the callback finds that the page is not on lru_ldwrt_hdr anymore and hits the assertion.
#
# - Workaround: Script turns off the buddy cmp for flushing to workaround the issue.
#
# - Defect(s) it works around: 134766,157595

Version=1.00

# For EGA specify GA and EMUx specify MUx
TPD_VERSIONS="3.2.1.MU[2345]"
...
usage()
{
  ...
  echo -e "\nNote: On upgrade failure or revert uninstall the script by using --uninstall option"
...
enable_turn_off_noblk_flush()
{
  local OPT="$1"

  local TPDVARTYPE=kernel # It can be kvar, mvar, dvar, kernel
  local TPDVARNAME=turn_off_noblk_flush
  local TPDVARSIZE=32  # For kvar,mvar,dvar pass "xx"
  local DEFAULTVAL=0
  local NEWVAL=1
  local DEFECTS="134766,157595" # No spaces in between
  local DESCRIPTION="Turning off the buddy cmp for flushing"
...
if [ "$OPT" != "" ]; then
  enable_turn_off_noblk_flush $OPT
else
