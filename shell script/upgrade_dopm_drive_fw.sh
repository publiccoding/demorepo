...
#
# upgrade_dopm_drive_fw.sh: Upgrade DOPM drives firmware if they are loaded with $FW_REV firmware version or lower than 3P05.
# Bug(s) Addressed: 199754, 212401, 211833
#
# Note:
# - If /tmp/upgrade_dopm_drive_fw.prompt file is present, script waits until user responds before proceeding to next drive.
...
#  2) For each DOPM drive, check recycle stall. If any drive matches with 'VPC mismatch' pattern then add to list drives need to be replaced.
...
#  4) Get list of SanDisk DOPM drives with lower than 3P05 firmware loaded.
...
#
# - Table below is used while checking DOPM drive VPC mismatch:
# ----------------------------------------------------------------------------------------------
# | DOPM FW  | Max Length | Header check? | VPC Pattern in drive event log                     |
# |----------|------------|---------------|----------------------------------------------------|
# | 3P01     | 0x80008    | Yes, needed   | ab 02 10 00 00 25 20 .1 .. .. .. .. .. 0b 00 00 00 |
# | 3P03     | 0x80008    | Yes, needed   | ab 02 10 00 00 25 20 .1 .. .. .. .. .. 0b 00 00 00 |
# | 3P04     | 0x80008    | Yes, needed   | ab 02 10 00 00 25 20 .1 .. .. .. .. .. 0b 00 00 00 |
# | 3P05     | 0x80008    | Yes, needed   | ab 02 10 00 00 25 20 .1 .. .. .. .. .. 0b 00 00 00 |
# ----------------------------------------------------------------------------------------------
#
...
# DOPM drives target firmware is 3P05
# - It is applicable for 3.2.1.MU3+P57 3.2.1.MU5+P59 3.2.2.MU2+P53 3.2.2.MU4+P57 3.2.2.MU3+P61
TPD_VERSIONS="3.2.1.MU[35]|3.2.2.MU[2-9]|3.3.1"

TOUCHFILESDIR="/common/touchfiles"
NO_SSD_UNMAP=$TOUCHFILESDIR/no_ssd_unmap

SCRIPT=upgrade_dopm_drive_fw.sh
...
usage()
{
    ...
    echo -e "--install : Upgrade DOPM drive(s) from $FW_REV firmware version."
    echo -e "--verify  : Verify the number of DOPM drive(s) by firmware version.\n"
...
# Upgrade each DOPM drive, if FW version is $FW_REV
upgrade_dopm_drive_fw()
{
    local opt=$1
    local retval=0
...
# check whether given DOPM drive has VPC mismatch pattern in drive event logs
# Return values: 0: If no VPC pattern found. 1: If VPC pattern found.
is_it_vpc_mismatch_dopm_pd()
{
    local pd_id=$1
    local pd_wwn=$2
    local pd_serial=$3
    ...
    # For DOPM vpc_pattern is common for all drive FW versions
    ...
    match_dopm_drive_header_serialnum $pd_id $pd_node_wwn $pd_serial $pd_fw
...
match_dopm_drive_header_serialnum()
{
    local pd_id=$1
    local pd_wwn=$2
    local pd_serial=$3
...
check_recycle_stall()
{
    ...
    while [ $current_cnt -lt $drive_count ]; do
        ...
        is_it_vpc_mismatch_dopm_pd $pd_id $pd_node_wwn $pd_serial $pd_fw_rev
        retval=$?
        if [ $retval -ne 0 ]; then
            retval=1
